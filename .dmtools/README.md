...
Ещё один способ распределить провайдера — не через `rule.provider`, а
списком в самой переменной: `AI_AGENT_PROVIDER=claude-code,codex`. Правило без
явного `"provider"` тогда не наследует список буквально (`run-agent.sh` не
умеет несколько провайдеров в одном job'е — это взаимоисключающие ветки CLI),
а `smAgent.js` сам выбирает первого из списка, у кого ещё остался бюджет (см.
ниже), и передаёт этот выбор в `workflow_dispatch` как один конкретный
`inputs.provider`.
...
```json
"maxTriggeredWorkflows": { "claude-code": 5, "codex": 1 }
```
— отдельный потолок на каждый провайдер: пока Codex занял свой единственный
слот, Claude Code продолжает диспатчить до 5 параллельных job'ов, не дожидаясь
Codex. Именно эта форма нужна для «параллельный Claude + Codex» — числовая
форма считает по всем провайдерам вместе и не разделяет очередь.
