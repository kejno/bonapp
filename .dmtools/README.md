...
а `smAgent.js` сам распределяет тикеты между провайдерами из списка **в
пределах одного прохода** (см. ниже) и передаёт выбор для каждого тикета в
`workflow_dispatch` как свой конкретный `inputs.provider`.
