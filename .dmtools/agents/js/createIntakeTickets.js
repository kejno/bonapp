 ...
 * the CLI agent writes per-ticket-prefixed paths).
 * @returns {Array} Array of story/epic entries, or empty array on error
 */
function readStoriesJson(ticketKey) {
    try {
        var raw = outputFiles.readOutputFile('stories.json', { ticketKey: ticketKey });
        if (!raw || raw.trim() === '') {
 ...
 */
function readCommentFile(ticketKey) {
    try {
        var content = outputFiles.readOutputFile('comment.md', { ticketKey: ticketKey });
 ...
 * @param {string} [ticketKey] - Source ticket key, for per-ticket path fallback
 * @returns {string} Description content
 */
function readDescriptionFile(filePath, fallbackSummary, ticketKey) {
    if (!filePath) {
        return fallbackSummary || '';
    }
    try {
        var content = outputFiles.readOutputFile(filePath, { ticketKey: ticketKey });
        if (content && content.trim() !== '') {
            return content;
        }
    } catch (error) {
 ...
 */
function createBug(entry, projectKey, sourceTicketKey) {
    var summary = buildSummary(entry.summary, 0);
    var description = readDescriptionFile(entry.description, summary, sourceTicketKey);
 ...
 * @returns {string|null} Created ticket key or null on failure
 */
function createEpic(entry, projectKey, sourceTicketKey) {
    var summary = buildSummary(entry.summary, 0);
    var description = readDescriptionFile(entry.description, summary, sourceTicketKey);
 ...
 * @returns {string|null} Created ticket key or null on failure
 */
function createStory(entry, resolvedParent, projectKey, sourceTicketKey) {
    var summary = buildSummary(entry.summary, 0);
    var description = readDescriptionFile(entry.description, summary, sourceTicketKey);
...
function action(params) {
    try {
        ...
        stories.forEach(function(entry) {
            ...
            var key = createBug(entry, projectKey, ticketKey);
            var summary = buildSummary(entry.summary, 0);
        ...
        stories.forEach(function(entry) {
            ...
            if (!entry.parent) {
                var key = createEpic(entry, projectKey, ticketKey);
                var summary = buildSummary(entry.summary, 0);
                if (key && entry.tempId) {
                    tempIdMap[entry.tempId] = key;
                    keyMap[entry.tempId] = key;
        ...
        stories.forEach(function(entry) {
            ...
            if (entry.parent) {
                ...
                var key = createStory(entry, resolvedParent, projectKey, ticketKey);
                var summary = buildSummary(entry.summary, 0);
                if (key) {
        ...
        // 5. Read AI comment
        var aiComment = readCommentFile(ticketKey);
