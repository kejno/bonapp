...
// Inlined rather than requiring a shared common/ module: this file also runs via
// smAgent.js's runLocalAction() (see "In Review Test Cases (pr_approved) → retry
// merge" in sm.json), which evals a hand-rolled require() shim that only resolves
// config.js/scm.js/configLoader.js by hardcoded id match — requiring any other
// module id (autoStart.js, tokenUsageComment.js included — see the requires above,
// which share this same latent issue) silently returns config.js's exports instead
// of throwing, so a call on the wrong object blows up with "is not a function" the
// first time it's actually invoked, not at require() time.
...
function action(params) {
    ...
    const pr = findPRForTicket(scm, branchKey);
    if (!pr) {
    ...
    if (mergeable === null ||
        ...
        if (mergeableState === 'unstable' && typeof scm.approveStuckActionRequiredRuns === 'function') {
            scm.approveStuckActionRequiredRuns(pr.head && pr.head.sha, owner, repo);
        }
        return false;
    }
