...
function mergeMain(storyKey, config) {
    ...
    try {
        var isShallow = cleanCommandOutput(runInRepo('git rev-parse --is-shallow-repository', workingDir) || 'false');
    ...
    // A prior step (detectMergeConflicts) may have left a `--no-commit --no-ff`
    // merge of origin/main staged but uncommitted (MERGE_HEAD present) in this
    // same working dir. `git merge` below would otherwise fail with
    // "You have not concluded your merge" even though there's no conflict —
    // finish that merge first so it doesn't get mistaken for a failed one.
    var mergeInProgress = false;
    try {
        mergeInProgress = cleanCommandOutput(runInRepo('git rev-parse -q --verify MERGE_HEAD', workingDir) || '').trim() !== '';
    } catch (e) {
        mergeInProgress = false;
    }
    if (mergeInProgress) {
        var pendingConflicts = cleanCommandOutput(runInRepo('git diff --name-only --diff-filter=U', workingDir) || '')
            .split('\n').map(function(s) { return s.trim(); }).filter(function(s) { return s; });
        if (pendingConflicts.length === 0) {
            console.log('Found a staged, uncommitted merge from a prior step — committing it before continuing');
            runInRepo('git commit -m "' + (storyKey + ' test: merge origin/main').replace(/"/g, '\\"') + '"', workingDir);
        }
    }
