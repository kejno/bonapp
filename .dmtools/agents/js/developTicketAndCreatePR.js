...
function createPullRequest(title, branchName, baseBranch) {
    ...
    // outputFiles.readOutputFileDetailed() may resolve the file via file_read(),
    // which is sandboxed to the JSRunner cwd (.dmtools/) and can return a plain
    // relative path like "outputs/response.md" that reads fine there — but the
    // `gh pr create --body-file` command below runs through cli_execute_command,
    // whose cwd is the repo root (or _workingDir), where that same relative path
    // doesn't exist. Re-resolve to an absolute path via the repo root so the
    // returned path is valid for both readers.
    function readFileDetailed(path) {
        var detailed = outputFiles.readOutputFileDetailed(path, { workingDir: _workingDir });
        if (!detailed || !detailed.path || detailed.path.indexOf('/') === 0) {
            return detailed;
        }
        // detailed.path is relative and may have resolved via file_read(),
        // which is sandboxed to the JSRunner's own cwd (.dmtools/ in this
        // workflow) — not the repo root that cli_execute_command/gh use. Try
        // both possible absolute locations and verify with `cat` before
        // handing one to `gh pr create --body-file`, instead of assuming the
        // repo-root twin exists (it may actually live under repoRoot/.dmtools/).
        try {
            var rootOut = runCmd({ command: 'git rev-parse --show-toplevel' }) || '';
            var repoRoot = cleanCommandOutput(rootOut).trim();
            if (repoRoot) {
                var candidates = [repoRoot + '/' + detailed.path, repoRoot + '/.dmtools/' + detailed.path];
                for (var i = 0; i < candidates.length; i++) {
                    try {
                        var catOut = runCmd({ command: 'cat "' + candidates[i] + '"' }) || '';
                        var catStr = cleanCommandOutput(String(catOut));
                        if (catStr.trim() && catStr.indexOf('No such file or directory') === -1) {
                            return { content: detailed.content, path: candidates[i] };
                        }
                    } catch (e) {}
                }
                console.warn('Neither absolute candidate exists on disk for PR body file — falling back to relative path:', candidates);
            }
        } catch (e) {
            console.warn('Could not resolve repo root for PR body file path:', e);
        }
        return detailed;
    }
