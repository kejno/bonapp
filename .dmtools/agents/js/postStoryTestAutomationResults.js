...
function readFileDetailed(path) {
    return outputFiles.readOutputFileDetailed(path, {});
}
...
function attemptResumeIfOutputsIncomplete(storyKey, result, linkedTestCases, workingDir) {
    ...
    //
    // CONFIRMED (via a debug probe: file_read() found promptPath right after
    // file_write() wrote it, while `find /` through cli_execute_command found
    // it nowhere on the filesystem): file_write()/file_read() resolve against
    // JSRunner's own cwd (.dmtools/, per this workflow's
    // working-directory: .dmtools), while cli_execute_command spawns a
    // separate OS process rooted at the repo root. So BOTH the run-agent.sh
    // script path and promptPath need the .dmtools/ prefix when handed to
    // cli_execute_command — the script because it physically lives under
    // .dmtools/agents/scripts/, and promptPath because that's where
    // file_write() actually put it.
    var command = 'bash .dmtools/agents/scripts/run-agent.sh --continue .dmtools/' + promptPath;
...
function createPullRequest(title, branchName, baseBranch, workingDir, scm) {
    ...
    return prHelper.createPullRequest({
        ...
        readFile: readFile,
        readFileDetailed: readFileDetailed
    });
}

// "No new staged changes on this branch" (performGitOperations' noNewCommit) only
...
function isBranchMergedOrHasNoPR(branchName, scm) {
    try {
        ...
        var openMatch = openPRs.some(function(pr) {
            return pr.head && pr.head.ref === branchName;
        });
        if (openMatch) {
            console.log('Branch', branchName, 'still has an open PR — not finalizing Test Case statuses yet');
            return false;
        }
        return true;
    } catch (e) {
        console.warn('Could not check PR state for branch', branchName, ':', e);
        // Fail open: if we can't tell, don't block finalization on an unrelated API hiccup.
        return true;
    }
}
...
function action(params) {
    try {
        ...
        // Whether it's safe to finalize Test Case statuses to Passed/Done in the
        // no-code-changes path — false when an open PR on this branch still needs
        // its own review/merge cycle (see isBranchMergedOrHasNoPR).
        ...
        if (branchName) {
            ...
            if (gitResult.success && !gitResult.noNewCommit) {
                ...
                noCodeChanges = true;
                readyToFinalize = isBranchMergedOrHasNoPR(branchName, scm);
                console.log(readyToFinalize
                    ? 'ℹ️ No test code changes and no open PR — skipping review, moving Story directly'
                    : 'ℹ️ No test code changes, but the branch still has an open PR — waiting for that review/merge cycle instead of finalizing');
            } else {
        ...
        if (result.results && result.results.length > 0) {
            result.results.forEach(function(item) {
                if (item.status === 'passed' || item.status === 'failed') {
                    ...
                    // When there are no test code changes AND no open PR is still pending
                    // review on this branch, the fix is already merged (or never needed
                    // one) — finalize immediately so story_done_check can act. If a PR is
                    // still open, leave the TC in In Review - Passed/Failed and let the
                    // normal review/merge cycle finalize it once that PR is actually merged.
                    if (readyToFinalize) {
        ...
        // Anti-cycle guard: the "In Testing Stories with an open test PR → review"
        // SM rule matches on Jira status alone, with no way to tell whether a PR
        ...
        if (readyToFinalize) {
            try {
                ...
                console.log('Added label', LABELS.TEST_PR_FINALIZED, 'to', storyKey);
            } catch (e) {
