...
- `apps/api/test/{TC_KEY}.e2e-spec.ts` for each linked Test Case

## Task steps

1. Address every comment in `outputs/review_replies.json`.
2. Make minimal, focused changes to `apps/api/test/` (or the relevant co-located `*.spec.ts`) only.
3. Re-run the affected tests via `npm run test:e2e -w apps/api` to confirm they still pass.
...
- Updated test code under `apps/api/test/{TC_KEY}.e2e-spec.ts`.
