...
describe('BNP-387: PDF for selected tables', () => {
  ...
  const pdfService = Object.create(TableQrPdfService.prototype) as TableQrPdfService;
  ...
  beforeAll(async () => {
    ...
    Object.assign(pdfService, { cache, prisma, onModuleInit: () => undefined, onModuleDestroy: () => undefined });
  ...
  afterAll(async () => {
    await app?.close();
    const browser = (pdfService as unknown as { browser?: { close: () => Promise<void> } }).browser;
    await browser?.close();
  });

  it('returns a valid PDF containing the selected tables only', async () => {
    const response = await request(app.getHttpServer() as never)
      .post('/api/v1/admin/tables/generate-qr-pdf')
      .send({ tableIds: tables.map(({ id }) => id) })
      .expect(200)
