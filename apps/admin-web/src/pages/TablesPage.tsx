      ...
      {selectedTable && <div className="fixed inset-0 z-10 flex items-center justify-center bg-black/40 p-4" role="presentation" onClick={() => setSelectedTable(null)}>
        <section role="dialog" aria-modal="true" aria-labelledby="table-details-title" onClick={(event) => event.stopPropagation()} className="w-full max-w-md rounded-2xl bg-surface-card p-6 shadow-xl">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <div><dt className="text-on-background/60">Метка</dt><dd>{selectedTable.label || '—'}</dd></div>
            <div><dt className="text-on-background/60">Мест</dt><dd>{selectedTable.seatsCount}</dd></div>
            <div><dt className="text-on-background/60">Статус</dt><dd>{STATUS[selectedTable.status]?.label ?? selectedTable.status}</dd></div>
            <div><dt className="text-on-background/60">Гость</dt><dd>{selectedTable.orders?.[0]?.guestSessionId ? 'Гость без учётной записи' : 'Не указан'}</dd></div>
