...
export default function StaffPage() {
  ...
  const load = async () => {
    if (!token) return;
    try { const [staff, currentShift, chefs] = await Promise.all([api<Employee[]>('/admin/staff', token), api<Shift>('/admin/shifts/current', token), api<KitchenStaff[]>('/admin/kitchen-staff', token)]); setEmployees(staff); setShift(currentShift); setKitchenStaff(chefs); setError(''); }
  ...
  const updateChefDepartments = async (chef: KitchenStaff, department: string) => {
    if (!token) return;
    const kitchenDepartments = chef.kitchenDepartments.includes(department)
      ? chef.kitchenDepartments.filter((value) => value !== department)
  ...
  return <main className="mx-auto max-w-6xl space-y-8 p-8 text-stone-900"><header className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Сотрудники и смены</h1><p className="mt-1 text-stone-500">Управление доступом команды ресторана</p></div><button className="rounded-xl bg-orange-600 px-4 py-2 text-white" onClick={() => setEditing({ id: '', fullName: '', email: '', phone: '', role: 'WAITER', isActive: true, lastLoginAt: null })}>Добавить сотрудника</button></header>
    ...
    <section className="rounded-2xl border bg-white p-6"><h2 className="mb-2 text-xl font-semibold">Цеха поваров для KDS</h2><p className="mb-4 text-sm text-stone-500">Назначьте каждому повару цеха, карточки которых он будет видеть и менять.</p>{kitchenStaff.length ? kitchenStaff.map((chef) => <fieldset key={chef.id} className="mb-4 flex flex-wrap items-center gap-4 border-b pb-4"><legend className="w-full font-medium">{chef.fullName}</legend>{KITCHEN_DEPARTMENTS.map(({ id, label }) => <label key={id} className="flex items-center gap-2"><input type="checkbox" checked={chef.kitchenDepartments.includes(id)} onChange={() => void updateChefDepartments(chef, id)} />{label}</label>)}</fieldset>) : <p className="text-stone-500">Сотрудники с ролью «Повар» пока не созданы.</p>}</section>
